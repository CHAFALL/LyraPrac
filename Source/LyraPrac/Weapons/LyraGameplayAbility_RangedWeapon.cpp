...
#include "LyraPrac/AbilitySystem/LyraGameplayAbilityTargetData_SingleTarget.h"
#include "LyraPrac/Physics/LyraCollisionChannels.h"
...
void ULyraGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	// - LyraAbilitySet::GiveToAbilitySystem()에서 GiveAbility()를 호출한다
...
FHitResult ULyraGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, TArray<FHitResult>& OutHits) const
{
	FHitResult Impact;

	// 우선 SweepRadius 없이 한번 Trace 진행한다 (SweepTrace는 무겁기 때문, 한 10배 더...)
